% TEST THE RLS CLASS - STATE ESTIMATION VERSION
%   > Tests RLS refactored to estimate state vector x (consistent with Kalman filtering)
...
switch sim
    ...
    case 4
        ...
        for n = 1:N
            ...
            for a = 1:Na
                y_sd = 10^(noisePowers_dB(a)/10);
                d(:,a,n,:) = y_true(n) + y_sd * randn(1,1,1,M);
            end
        end
...
fprintf('=== RLS State Estimation Test Configuration ===\n');
...
% xlabel('Time')
% ylabel('Observation')
% % title('Testing RLS State Estimation - Noisy Observations')
% hold on
% grid on

%% RLS State Estimation
fprintf('\n=== RLS State Estimation ===\n');

% RLS initialization with start_vals structure for state estimation
start_vals = struct();
start_vals.delta = 0.1;                    % Initial state covariance scaling
start_vals.initial_state = zeros(x_dim, 1); % Initial state estimate

% Create RLS instance for state estimation
if sim == 4
    H_ini = H_matrix(1,:);
else
    H_ini = H_matrix;
...
fprintf('RLS state estimation instance created successfully!\n');
...
for m = 1:M
    ...
    for n = 1:N
        ...
        [y_hat_out, x_hat_out] = rls.apply('measurement', measurement);
...
if M > 1
    ...
    title('RLS State Estimation: Predicted Observations (1st Realization)')
else
    plot(1:N, y_hat_history(1,:), 'r', 'LineWidth', 1.5)
    title('RLS State Estimation: Predicted Observations')
end
...
if M > 1
    title(sprintf('RLS: Mean Squared Error (Prediction) - Monte Carlo: %d realizations', M));
else
    title('RLS: Mean Squared Error (Prediction)')
end
ylabel('MSE [dB]')
xlabel('Time Step')
set(gca, 'YLim', [-70 10])
...
if M > 1
    title(sprintf('RLS: Mean Squared Deviation (State Estimation) - Monte Carlo: %d realizations', M));
else
    title('RLS: Mean Squared Deviation (State Estimation)')
end
ylabel('MSD [dB]')
xlabel('Time Step')
set(gca, 'YLim', [-70 10])
...
if M > 1
    for i = 1:x_dim
        ...
        grid on
        if i == 1
            title(sprintf('RLS: State Convergence, Monte Carlo: %d realizations', M));
        end
        if i == x_dim
            xlabel('Time Step')
        end
...
fprintf('\n=== RLS State Estimation Test Summary ===\n');
fprintf('Test completed successfully!\n');
fprintf('RLS class refactored for state estimation (consistent with Kalman filtering).\n');
