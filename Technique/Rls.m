    ...
    methods (Access = public)
        function obj = RLS(varargin)
            ...
            try
                ...
                if ~isscalar(obj.start_vals.delta) || obj.start_vals.delta <= 0
                    error('RLS: start_vals.delta must be a positive scalar');
                end

                if length(obj.start_vals.initial_state) ~= obj.x_dim
                    error('RLS: start_vals.initial_state must have length equal to x_dim');
                end
                ...
                error('An error occurred in RLS constructor: %s', exception.message);
            end
        ...
        function [varargout] = apply(obj, varargin)
            ...
            try
                ...
                if isempty(ind)
                    error('RLS: Measurement not provided.');
                else
                    y = varargin{ind+1};
                    if ~isnumeric(y) || length(y) ~= obj.y_dim
                        error('RLS: Measurement must be numeric with length equal to y_dim');
                    end
                ...
                switch nargout
                    case 0
                        fprintf('RLS: No output requested in RLS.\n');
                    case 1
        ...
        function obj = reset(obj)
            ...
            try
                ...
                error('RLS: An error occurred in RLS reset: %s', exception.message);
            end
        end
            ...
            try
                ...
                if ~isempty(ind)
                    ...
                    if all(size(H_new) == [obj.y_dim, obj.x_dim])
                        ...
                        error('RLS: H_matrix dimensions must be [y_dim, x_dim]');
                    end
                end
                ...
                if ~isempty(ind)
                    ...
                    if isscalar(lambda_new) && lambda_new > 0 && lambda_new <= 1
                        ...
                        error('RLS: lambda must be a scalar in (0, 1]');
                    end
                end
                ...
                if ~isempty(ind)
                    ...
                    if all(size(P_new) == [obj.x_dim, obj.x_dim])
                        ...
                        error('RLS: covariance_estimate dimensions must be [x_dim, x_dim]');
                    end
                end
                ...
                if ~isempty(ind)
                    ...
                    if length(x_new) == obj.x_dim
                        ...
                        error('RLS: state_estimate length must equal x_dim');
                    end
                end
        ...
        function obj = update_H_matrix(obj, H_new)
            ...
            if all(size(H_new) == [obj.y_dim, obj.x_dim])
                ...
                error('RLS: H matrix dimensions must be [y_dim, x_dim]');
            end
        end
        ...
        function obj = update_x_hat(obj, x_new)
            ...
            if length(x_new) == obj.x_dim
                ...
                error('RLS: State vector length must equal x_dim');
            end
        end
