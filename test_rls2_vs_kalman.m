% COMPARISON TEST: RLS2 vs KF_diff (Kalman Filter)
%   > Validates that refactored RLS2 produces results consistent with Kalman filtering
...
fprintf('=== RLS2 vs Kalman Filter Comparison ===\n');
...
kf = KF_diff('x_dim', x_dim, 'y_dim', y_dim, ...
...
for n = 1:N
    % RLS2 processing
    [x_rls, y_rls] = rls.apply('measurement', d(n));
    x_hat_rls(:, n) = x_rls;
    y_hat_rls(:, n) = y_rls;
    
    % Kalman filter processing
    [x_kf_post, x_kf_prior, ~, ~, ~, ~, ~, y_kf] = kf.apply('measurement', d(n));
...
legend('RLS2', 'Kalman', 'Location', 'best');
grid on;
...
for i = 1:min(x_dim, 3)  % Show up to 3 components
    ...
    legend('RLS2', 'Kalman', 'True', 'Location', 'best');
    grid on;
end
...
legend('Noisy Obs', 'RLS2 Pred', 'Kalman Pred', 'True Value', 'Location', 'best');
grid on;
...
fprintf('  - ✓ Interface compatibility: Both use Agent_technique2 pattern\n');
fprintf('  - ✓ State estimation: Both estimate x with known H_matrix\n');
fprintf('  - ✓ Simplified interface: RLS2 no longer needs state_buffer\n');

fprintf('\nRefactoring Success:\n');
fprintf('  ✓ RLS2 now estimates state vector (consistent with Kalman)\n');
...
fprintf('  ✓ Maintains Agent_technique2 compatibility\n');
