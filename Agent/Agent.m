classdef Agent < handle
    ...
    methods
        ...
        function update_agent_estimates(obj, varargin)
            ...
            %             else
            %                 warning('Agent: No fusion results to update agent estimates.')
            %             end
            %         otherwise
            %             error('Agent: Unknown source for updating agent estimates.')
            %     end
            % catch exception
            %     error('Agent: Error in update_agent_estimates - %s', exception.message);
            % end

            % if ~isempty(obj.fusion_results.state_estimate)
            if isfield(obj.fusion_results, 'state_estimate') || isfield(obj.fusion_results, 'covariance_estimate')
                obj.fusion_technique.update_agent_technique_params(obj);
            elseif isa(obj.fusion_technique, 'Noon_coop')
                % Do nothing
            else
                warning('Agent: No fusion results to update agent estimates.')
            end
        ...
        function obj = remove_agent(obj, agent_idx)
            ...
            if ~isempty(find_idx)
                ...
                error('Fusion_technique: Error in remove_agent - Agent not found.');
            end
        ...
        function P = get_prior_covariance(obj)
            ...
            if isa(obj.agent_technique, 'Kalman') && isprop(obj.agent_technique, 'Pa')
        ...
        function K = get_kalman_gain(obj)
            ...
            if isa(obj.agent_technique, 'Kalman') && isprop(obj.agent_technique, 'K')
